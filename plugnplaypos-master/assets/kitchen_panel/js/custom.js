...
$(document).ready(function(){
	...
	$(document).on('click','.select_all_of_an_order',function(){
		...
		$('#single_order_'+order_id+' .items_holder .single_item').find('.tags').css('color','#fff');
	...
	$(document).on('click','.unselect_all_of_an_order',function(){
		...
		$('#single_order_'+order_id+' .items_holder .single_item').find('.tags').css('color','');
		...
		$('#single_order_'+order_id+' .items_holder .single_item[data-cooking-status="Done"]').find('.tags').css('color','#fff');
		...
		$('#single_order_'+order_id+' .items_holder .single_item[data-cooking-status="Started Cooking"]').find('.tags').css('color','#fff');
	                            ...
	                            $order_list_left += '<div data-selected="unselected" class="fix single_item '+item_background+'" data-order-id="'+response[key].sales_id+'" data-item-id="'+single_item.previous_id+'" id="detail_item_id_'+single_item.previous_id+'" data-cooking-status="'+single_item.cooking_status+'">';
	                                $order_list_left += '<div class="single_item_left_side fix">';
		                                ...
		                                $order_list_left += '<div class="fix floatleft item_detail">';
											...
											var tags = single_item.tags;
		                                    tags_length = tags.length;
		                                    var w = 1;
		                                    var tags_name = '';
		                                    for(var key_tag in tags){
		                                       if(w==tags_length){
													tags_name += tags[key_tag].item_title;
		                                       }else{
													tags_name += tags[key_tag].item_title + ', ';
		                                       }
		                                       w++; 
		                                    ...
		                                    if(tags_length>0){
		                                        $order_list_left += '<p class="tags" style="'+font_style+'">- '+tags_name+'</p>';    
		                                    }
