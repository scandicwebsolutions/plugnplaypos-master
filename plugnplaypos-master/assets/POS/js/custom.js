...
$(document).ready(function () {
	...
	first_tableid($('#init_tableid').val());
	$('.bottom_add').on('click', function () {
		var table_id = $(this).attr('id').substr(38);
		first_tableid(table_id);
	});
	function first_tableid(table_id){
	...
	$(document).on('click', '.edit_item', function () {
		...
		var modifiers_price = parseFloat(0).toFixed(2);
		var tags_price = parseFloat(0).toFixed(2);
		...
		if (single_order_element_object.find('#item_tags_price_table_' + menu_id).length > 0) {
			var comma_separeted_tags_price = single_order_element_object.find('#item_tags_price_table_' + menu_id).html();
			var tags_price_array = (comma_separeted_tags_price != "") ? comma_separeted_tags_price.split(",") : '';
			tags_price_array.forEach(function (tag_price) {
				tags_price = (parseFloat(tags_price) + parseFloat(tag_price)).toFixed(2);
			});
			parseFloat(single_order_element_object.find('#item_tags_price_table_' + menu_id).html()).toFixed(2);
		}
		...
		if (single_order_element_object.find('#item_tags_id_table_' + menu_id).length > 0) {
			var comma_seperted_tags_id = single_order_element_object.find('#item_tags_id_table_' + menu_id).html();
			var tags_id = (comma_seperted_tags_id != "") ? comma_seperted_tags_id.split(",") : '';
		}

		var modifiers_price_as_per_item_quantity = (parseFloat(modifiers_price) * parseFloat(item_quantity)).toFixed(2);
		var tags_price_as_per_item_quantity = (parseFloat(tags_price) * parseFloat(item_quantity)).toFixed(2);
		var total_price = (parseFloat(item_price_with_discount) + parseFloat(modifiers_price_as_per_item_quantity) + parseFloat(tags_price_as_per_item_quantity)).toFixed(2);
		...
		$('#modal_tags_unit_price_variable').html(tags_price);
		$('#modal_tags_price_variable').html(tags_price_as_per_item_quantity);
		...
		var originalTag = foundItems[0].tags;		
		var tags = '';
		for (var key in originalTag) {
			tags += '<div class="modal_tags" id="menu_tag_' + originalTag[key].menu_tag_id + '">';
			tags += '<p>' + originalTag[key].menu_tag_title + '</p>';

			if (originalTag[key].menu_tag_type === 'Text Field') {
				tags += '<input type="text" class="tag_input" name="items_' + originalTag[key].menu_tag_id + '" placeholder="' + originalTag[key].menu_tag_title + '" />';
			} else {
				tags += '<div class="form-group" style="text-align:left">';
				var tag_items = originalTag[key].menu_tag_items;
				for (var ik in tag_items) {
					var checkedOrNot = "";
					if (typeof tags_id !== 'undefined' && tags_id.includes(originalTag[key].menu_tag_id + '*' + tag_items[ik].tag_item_id)) {
						checkedOrNot = "checked";
					}

					tags += '<div class="tag_line">';
					tags += '<label>';
					if (originalTag[key].menu_tag_type === 'Radio Group') {
						tags += '<input type="radio" class="tag_radio" ';						
					} else {
						tags += '<input type="checkbox" class="tag_checkbox" ';
					}
					tags += 'name="items_' + originalTag[key].menu_tag_id + '" ';
					tags += 'data-tag-id="' + originalTag[key].menu_tag_id + '" ';
					tags += 'data-item-id="' + tag_items[ik].tag_item_id + '" ';
					tags += 'data-title="' + tag_items[ik].tag_item_title + '" ';
					tags += 'value="' + tag_items[ik].tag_item_price + '" ';
					tags += checkedOrNot + '/>';
					tags += tag_items[ik].tag_item_title + ' ($' + tag_items[ik].tag_item_price + ')';
					tags += '</label></div>';
				}
				tags += '</div>'
			}
		...
		$("#item_modal .section_t2").empty();
		$("#item_modal .section_t2").prepend(tags);
	...
	$(document).on('click', '.tag_radio', function () {
		if (radioState === this) {
            this.checked = false;
            radioState = null;
        } else {
		...
		var total_tags_price = calc_tags_price();
		$('#modal_tags_unit_price_variable').html(total_tags_price);
		update_all_total_price();
	});

	// $(document).on('change', '.tag_radio', function () {
	// 	var total_tags_price = calc_tags_price();
	...
	$(document).on('change', '.tag_checkbox', function () {
	...
	$('#add_to_cart').on('click', function () {
		...
		var selected_tags_price = '';
		var j = 1;
		$('.modal_tags input:checked').each(function (i, obj) {
			if (j == $('.modal_tags input:checked').length) {
				selected_tags += $(this).attr('data-title');
				selected_tags_ids += $(this).attr('data-tag-id') + '*' + $(this).attr('data-item-id');
				selected_tags_price += $(this).val();
			} else {
				selected_tags += $(this).attr('data-title') + ',';
				selected_tags_ids += $(this).attr('data-tag-id') + '*' + $(this).attr('data-item-id') + ',';
				selected_tags_price += $(this).val() + ',';
		...
		//get tags price
		var tags_price = parseFloat($('#modal_tags_price_variable').html()).toFixed(2);
		...
		if (selected_tags != "") {
			draw_table_for_order += '<div class="forth_portion fix">';
			draw_table_for_order += '<span id="item_tags_id_table_' + item_id + '" style="display:none;">' + selected_tags_ids + '</span>';
			draw_table_for_order += '<span id="item_tags_price_table_' + item_id + '" style="display:none;">' + selected_tags_price + '</span>';
			draw_table_for_order += '<div class="single_order_column first_column fix"><span id="item_tags_table_' + item_id + '">' + selected_tags + '</span></div>';
			draw_table_for_order += '<div class="single_order_column fifth_column fix">' + currency + ' <span id="item_tags_price_table_' + item_id + '">' + tags_price + '</span></div>';
			draw_table_for_order += '</div>';
		}
		if (note != "") {
			draw_table_for_order += '<div class="third_portion fix">';
		...
		if (selected_order_type_object.length > 0) {
			...
			if ($('.order_holder .single_order').length > 0) {
				...
				$('.order_holder .single_order').each(function (i, obj) {
					...
					if ($(this).find('.forth_portion').length > 0) {
						var tags_id = $(this).find('#item_tags_id_table_' + item_id).html();
						var tags_price = $(this).find('#item_tags_price_table_' + item_id).html();
						items_info += ',"tags_id":"' + tags_id + '", "tags_price":"' + tags_price + '"';
					} else {
						items_info += ',"tags_id":"", "tags_price":""';
					}
	...
	$('#finalie_order_payment_method').change(function () {
		if ($(this).find("option:selected").text() == 'IPP') {
			$('#finalize_order_ipp_section').show();
			$('#finalize_order_cash_section').hide();
		} else {
			$('#finalize_order_cash_section').show();
			$('#finalize_order_ipp_section').hide();
		}
	}).change();

	$('.ipp_button').click(function () {
		$('#finalize_order_ipp_manual').hide();
		$('#finalize_order_ipp_emv').hide();
		$('#finalize_order_ipp_swipe').hide();

		var id = $(this).attr('data-id');
		if (id==1) {
			$('#finalize_order_ipp_manual').show();
		} else if (id==2) {
			$('#finalize_order_ipp_emv').show();
		} else {
			$('#finalize_order_ipp_swipe').show();
		}

		$('#selected_ipp_method').val(id);
		$('#finalize_order_button_ipp_div').css('display', 'inline-block');
	});

	$('#finalize_order_button_ipp').click(function () {
		var id = $('#selected_ipp_method').val();

		if (id==1) {
			finalize_order_ipp_manual();
		} else if (id==2) {
			finalize_order_ipp_emv();
		} else {
			finalize_order_ipp_swipe();
		}

	});

	function startLoading() {
		$('#modal_finalize_order_details').waitMe({
			effect: "win8",
			text: "Please wait...",
	...
	function stopLoading() {
		$('#modal_finalize_order_details').waitMe("hide");
	}

	function finalize_order_ipp_manual() {
		var sale_id = $('.holder .order_details .single_order[data-selected=selected]').attr('id').substr(6);
		var amount = $('#finalize_total_payable').html();
		...
		var card_num = $('#ipp_manual_card_num').val();
		var card_holder = $('#ipp_manual_card_holder').val();
		var card_expiry = $('#ipp_manual_card_expiry').val();
		var card_cvc = $('#ipp_manual_card_cvc').val();

		startLoading();

		$.ajax({
			url: base_url + "Sale/ipp_payment",
			method: "POST",
			data: {
				type: 'manual',
				order_id: sale_id,
			...
			success: function (response) {
				...
				stopLoading();
				if (res.status == 200) {
					print_invoice_and_close(sale_id, payment_method_type, invoice_create_type, amount, due_amount);
					reset_finalize_modal();
				} else {
					var error = JSON.parse(res.output);
					swal({
						title: warning + "!",
						text: error.message,
						confirmButtonText: ok,
	...
	function finalize_order_ipp_emv() {
		var sale_id = $('.holder .order_details .single_order[data-selected=selected]').attr('id').substr(6);
		var amount = $('#finalize_total_payable').html();
		var payment_method_type = $('#finalie_order_payment_method').val();
		var invoice_create_type = $('#finalize_update_type').html();
	...
	function finalize_order_ipp_swipe() {
		var sale_id = $('.holder .order_details .single_order[data-selected=selected]').attr('id').substr(6);
		var amount = $('#finalize_total_payable').html();
		var payment_method_type = $('#finalie_order_payment_method').val();
		var invoice_create_type = $('#finalize_update_type').html();
...
function calc_tags_price(selected) {
	var price = 0;

	$('.modal_tags input:checked').each(function() {
...
function update_all_total_price() {
	...
	//get tags unit sum price
	var tags_unit_sum_price = parseFloat($('#modal_tags_unit_price_variable').html()).toFixed(2);

	//set tags price as per item quantity 
	var tags_price = (parseFloat(tags_unit_sum_price) * parseFloat(item_quantity)).toFixed(2);
	$('#modal_tags_price_variable').html(tags_price);

	//add items and modifiers price
	var all_price = (parseFloat(item_price_after_discount) + parseFloat(modifiers_price) + parseFloat(tags_price)).toFixed(2);
	...
	$.ajax({
		...
		success: function (response) {
			...
			for (var key in response.items) {
				...
				var selected_tags_price = '';
				var tags_price = 0;
				var total_tag = this_item.tags.length;
				var i = 1;
				for (var tag_key in this_item.tags) {
					var this_tag = this_item.tags[tag_key];
					//get selected tag
					if (i == total_tag) {
						selected_tags += this_tag.item_title;
						selected_tags_id += this_tag.tag_id + '*' + this_tag.tag_item_id;
						selected_tags_price += this_tag.tag_price;
						tags_price = (parseFloat(tags_price) + parseFloat(this_tag.tag_price)).toFixed(2);
					} else {
						selected_tags += this_tag.item_title + ',';
						selected_tags_id += this_tag.tag_id + '*' + this_tag.tag_item_id + ',';
						selected_tags_price += this_tag.tag_price + ',';
				...
				var selected_tags_array = selected_tags.split(",");
				var tags_divs = '';
				var tags_length = selected_tags_array.length;
				if (tags_length > 0) {
					for (var index = 0; index < tags_length; index++) {
						tags_divs += (selected_tags_array[index] != "") ? '<span>' + selected_tags_array[index] + '</span>, ' : '';
					}
				}
				...
				if (this_item.item_type != "Bar Item") {
					...
					draw_table_for_kot_modal += tags_divs ? '<br>Tags: ' + tags_divs : '';
...
function set_all_items_tags_amount() {
	$('.order_holder .single_order').each(function (i, obj) {
		var tags_price = parseFloat(0).toFixed(2);
		var item_id = $(this).attr('id').substr(15);
		...
		if ($(this).find('#item_tags_price_table_' + item_id).length > 0) {

			var comma_separeted_tags_price = $(this).find('#item_tags_price_table_' + item_id).html();
			var tags_price_array = (comma_separeted_tags_price != "") ? comma_separeted_tags_price.split(",") : '';
			tags_price_array.forEach(function (tag_price) {
				tags_price = (parseFloat(tags_price) + parseFloat(tag_price)).toFixed(2);
			});
			var tags_price_as_per_item_quantity = (parseFloat(item_quantity) * parseFloat(tags_price)).toFixed(2)
			$(this).find('.fifth_column #item_tags_price_table_' + item_id).html(tags_price_as_per_item_quantity);
		}
	});
}
...
function get_total_of_all_items_and_modifiers() {
	...
	//get all tags price and add
	var all_tags_total_price = 0;
	$('.single_order .forth_portion .fifth_column span').each(function (i, obj) {
		all_tags_total_price = (parseFloat(all_tags_total_price) + parseFloat($(this).html())).toFixed(2);
	});

	return (parseFloat(all_item_total_price) + parseFloat(all_modifiers_total_price) + parseFloat(all_tags_total_price)).toFixed(2);
}
...
function reset_on_modal_close_or_add_to_cart() {
	...
	$('#modal_tags_price_variable').html('0');
	$('#modal_tags_unit_price_variable').html('0');
	$('#modal_item_note').val('');
	$('#modal_discount').val('');
	...
	$.ajax({
		...
		success: function (response) {
			...
			for (var key in response.items) {
				...
				if (selected_tags != "") {
					draw_table_for_order_details += '<div class="forth_portion fix">';
					draw_table_for_order_details += '<span id="order_details_item_tags_id_table_' + this_item.food_menu_id + '" style="display:none;">' + selected_tags_id + '</span>';
					draw_table_for_order_details += '<span id="order_details_item_tags_price_table_' + this_item.food_menu_id + '" style="display:none;">' + selected_tags_price + '</span>';
					draw_table_for_order_details += '<div class="single_order_column_hold first_column column fix"><span id="order_details_item_tags_table_' + this_item.food_menu_id + '">' + selected_tags + '</span></div>';
					draw_table_for_order_details += '<div class="single_order_column_hold second_column column fix">' + currency + ' <span id="order_details_item_tags_price_table_' + this_item.food_menu_id + '">' + tags_price + '</span></div>';
					draw_table_for_order_details += '</div>';
				}
				if (this_item.menu_note != "") {
					draw_table_for_order_details += '<div class="third_portion fix">';
...
function reset_finalize_modal() {
	...
	$('#ipp_manual_card_num').val('');
	$('#ipp_manual_card_holder').val('');
	$('#ipp_manual_card_expiry').val('');
	$('#ipp_manual_card_cvc').val('');

	$("#ipp_emv_list").prop("selectedIndex", 0);
	$('#magstripe').val('');

	$('#selected_ipp_method').val('');

	$('#finalize_order_ipp_manual').hide();
	$('#finalize_order_ipp_emv').hide();
	$('#finalize_order_ipp_swipe').hide();

	$('#finalize_order_button_ipp_div').css('display', 'none');
}
...
function arrange_info_on_the_cart_to_modify(response) {
	...
	for (var key in response.items) {
		...
		if (selected_tags != "") {
			draw_table_for_order += '<div class="forth_portion fix">';
			draw_table_for_order += '<span id="item_tags_id_table_' + this_item.food_menu_id + '" style="display:none;">' + selected_tags_id + '</span>';
			draw_table_for_order += '<span id="item_tags_price_table_' + this_item.food_menu_id + '" style="display:none;">' + selected_tags_price + '</span>';
			draw_table_for_order += '<div class="single_order_column first_column fix"><span id="item_tags_table_' + this_item.food_menu_id + '">' + selected_tags + '</span></div>';
			draw_table_for_order += '<div class="single_order_column fifth_column fix">' + currency + ' <span id="item_tags_price_table_' + this_item.food_menu_id + '">' + tags_price + '</span></div>';
			draw_table_for_order += '</div>';
		}
		if (this_item.menu_note != "") {
			draw_table_for_order += '<div class="third_portion fix">';
