...
function itemsCountByTag($food_tag_id) {
    $CI = & get_instance();
    $count = $CI->db->query("SELECT * FROM tbl_food_tags_items where `tag_id`='$food_tag_id'")->num_rows();
    return $count;
}

function tagsCountByFoodMenu($food_menu_id) {
    $CI = & get_instance();
    $count = $CI->db->query("SELECT * FROM tbl_food_menus_tags where `food_menu_id`='$food_menu_id'")->num_rows();
    return $count;
}
