...
class Authentication extends Cl_Controller {
    ...
    public function index() {
        ...
        if (!$this->session->userdata('login_type')) {
            $this->session->set_userdata('login_type', 'pin');
        }

        $data = array();
        if ($this->session->userdata('login_type') == "pin") {
            $data['login_content'] = $this->load->view('authentication/login.pin.php', null, TRUE);
        } else {
            $data['login_content'] = $this->load->view('authentication/login.email.php', null, TRUE);
        }            
        $this->load->view('authentication/login', $data);
    }

    public function chooseLoginType($login_type) {
    ...
    public function loginCheck() {
        ...
        if ($this->input->post('login_type') == "pin") {
            $this->form_validation->set_rules('pin_code', lang('pin_code'), "required|exact_length[4]");
            if ($this->form_validation->run() == TRUE) {
                $pin = $this->input->post($this->security->xss_clean('pin_code'));
                $user_information = $this->Authentication_model->getUserInformationByPin($pin);
            } else {
                $data['login_content'] = $this->load->view('authentication/login.pin.php', null, TRUE);
                $this->load->view('authentication/login', $data);
                return;
        ...
        if ($user_information) {
            ...
            if ($user_information->active_status == 'Active') {
                ...
                $login_session['pin_code'] = $user_information->pin_code;
