...
class Sale extends Cl_Controller {
    ...
    private function makeTagItemsArray($tagItems) {
        $itemsArray = array();

        foreach ($tagItems as $item) {
            if (!array_key_exists($item->tag_id, $itemsArray)) {
                $itemsArray[$item->tag_id] = array();
            }
            $itemsArray[$item->tag_id][] = $item;
        }
    ...
    public function POS($encrypted_id = "") {
        ...
        $data['seltable_id'] = $this->input->post('table_id');
        $data['seluser_id'] = $this->Common_model->Compareuser($username);
        ...
        $data['menu_tags'] = $this->Sale_model->getAllMenuTags();
        $data['menu_tag_items'] = $this->makeTagItemsArray($this->Sale_model->getAllMenuTagItems());
        ...
        $data['ipp_methods'] = $this->getIPPMethod();
        $data['emv_device_list'] = $this->getEMVDeviceList();
    ...
    private function getIPPMethod() {
        $company_id = $this->session->userdata('company_id');
        $ipp_settings = $this->Common_model->getDataById(6, "tbl_payment_methods");
        $ipp_items = explode("||", $ipp_settings->ipp_methods);

        $ipp_methods = array();

        foreach ($ipp_items as $item_id) {
            $ipp_methods[] = $this->Master_model->getIPPMethodById($item_id, $company_id);
        }

        return $ipp_methods;
    }

    private function getEMVDeviceList() {
        $ipp_settings = $this->Common_model->getDataById(6, "tbl_payment_methods");
    ...
    public function ipp_payment() {
        ...
        if ($type == 'manual') {
            ...
            $data['cardNumber'] = $card_num;
            $data['cardHolder'] = $card_holder;
            $data['cardExpire'] = $card_expiry;
            $data['cardCVC'] = $card_cvc;
        } else if ($type == 'emv') {
            $emv_id = $this->input->post('emv_id');

            $data['emvID'] = $emv_id;
        } else {
            $magStripe = $this->input->post('magStripe');
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $ipp_settings->ipp_endpoint_url . "/payment");
        ...
        $result = array("status" => $httpcode, "output" => $output);
    ...
    function add_sale_by_ajax(){
        ...
        if($sale_id>0){
            ...
            $this->db->delete('tbl_sales_details_tags', array('sales_id' => $sale_id));
        ...
        if($sales_id>0 && count($order_details->items)>0){
            foreach($order_details->items as $item){
                ...
                $tags_id_array = $item->tags_id != "" ? explode(",", $item->tags_id) : null;
                $tags_price_array = $item->tags_price != "" ? explode(",", $item->tags_price) : null;

                if(!empty($tags_id_array)>0){
                    $i = 0;
                    foreach($tags_id_array as $single_tag_id){
                        $tag_data = array();
                        $_tag_id = explode("*", $single_tag_id);
                        $tag_data['tag_id'] = $_tag_id[0]; 
                        $tag_data['tag_item_id'] = $_tag_id[1];
                        $tag_data['tag_price'] = $tags_price_array[$i]; 
                        $tag_data['food_menu_id'] = $item->item_id; 
                        $tag_data['sales_id'] = $sales_id; 
                        $tag_data['sales_details_id'] = $sales_details_id; 
                        $tag_data['user_id'] = $this->session->userdata('user_id');
                        $tag_data['outlet_id'] = $this->session->userdata('outlet_id'); 
                        $tag_data['customer_id'] =$order_details->customer_id;
                        
                        $this->db->insert('tbl_sales_details_tags', $tag_data);
    ...
    function get_all_information_of_a_sale($sales_id){
        ...
        foreach($items_by_sales_id as $single_item_by_sale_id){
            $tag_information = $this->Sale_model->getTagsBySaleAndSaleDetailsId($sales_id,$single_item_by_sale_id->sales_details_id);
            $single_item_by_sale_id->tags = $tag_information;
        }
            ...
            foreach($order_details->items as $item){
                ...
                if(!empty($tags_id_array)>0){
                    ...
                    foreach($tags_id_array as $single_tag_id){
                        ...
                        $tag_data['tag_price'] = $tags_price_array[$i]; 
                        $tag_data['food_menu_id'] = $item->item_id; 
                        $tag_data['holds_id'] = $holds_id; 
                        $tag_data['holds_details_id'] = $holds_details_id; 
                        $tag_data['user_id'] = $this->session->userdata('user_id');
                        $tag_data['outlet_id'] = $this->session->userdata('outlet_id'); 
                        $tag_data['customer_id'] =$order_details->customer_id;
                        
                        $this->db->insert('tbl_holds_details_tags', $tag_data);
    ...
    public function get_single_hold_info_by_ajax()
    {
        ...
        foreach($items_by_holds_id as $single_item_by_hold_id){
            $tag_information = $this->Sale_model->getTagsByHoldAndHoldsDetailsId($hold_id,$single_item_by_hold_id->holds_details_id);
            $single_item_by_hold_id->tags = $tag_information;
        }
    ...
    public function delete_all_information_of_hold_by_ajax()
    {
        ...
        $this->db->delete('tbl_holds_details_tags', array('holds_id' => $hold_id));    
    }
    ...
    public function delete_all_holds_with_information_by_ajax()
    {
        ...
        $this->db->delete('tbl_holds_details_tags', array('user_id' => $user_id,'outlet_id' => $outlet_id));
        echo 1;
    }
