...
class User extends Cl_Controller {
    ...
    public function addEditUser($encrypted_id = "") {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($id != '') {
                $post_pin = $this->input->post($this->security->xss_clean('pin_code'));
                $existing_pin = $user_details->pin_code;
                if ($post_pin != $existing_pin) {
                    $this->form_validation->set_rules('pin_code',  lang('pin_code'), "required|exact_length[4]|is_unique[tbl_users.pin_code]");
                } else {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $user_info['pin_code'] = $this->input->post($this->security->xss_clean('pin_code'));
