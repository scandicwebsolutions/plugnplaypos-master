...
class Master extends Cl_Controller {
    ...
    public function saveFoodMenusTags($food_menu_tags, $food_menu_id) {
        foreach ($food_menu_tags as $row => $tag_id):
            $fmt = array();
            $fmt['tag_id'] = $tag_id;
            $fmt['food_menu_id'] = $food_menu_id;
            $fmt['user_id'] = $this->session->userdata('user_id');
            $fmt['company_id'] = $this->session->userdata('company_id');
            $this->Common_model->insertInformation($fmt, "tbl_food_menus_tags");
        endforeach;
    }

    public function assignFoodMenuTag($encrypted_id = "") {
        $id = $this->custom->encrypt_decrypt($encrypted_id, 'decrypt');
        $company_id = $this->session->userdata('company_id');

        $food_menu_tags = $this->Master_model->getFoodMenuTags($id);
        if (!empty($food_menu_tags)) {
            foreach ($food_menu_tags as $value) {
                $user_menu_tags_arr[] = $value->tag_id;
            }
        } else {
            $user_menu_tags_arr = '';
        }
        ...
        if ($this->input->post('submit')) {
            $this->Common_model->deletingMultipleFormData('food_menu_id', $id, 'tbl_food_menus_tags');
            $this->saveFoodMenusTags($_POST['tag_id'], $id, 'tbl_food_menus_tags');
            $this->session->set_flashdata('exception', 'Information has been updated successfully!');
            redirect('Master/foodMenus');
        } else {
            $data['encrypted_id'] = $encrypted_id;
            $data['tags'] = $this->Master_model->getFoodTags($company_id);
            $data['food_menu_details'] = $this->Common_model->getDataById($id, "tbl_food_menus");
            $data['food_menu_tags'] = $user_menu_tags_arr;
            $data['main_content'] = $this->load->view('master/foodMenu/assignFoodMenuTag', $data, TRUE);
            $this->load->view('userHome', $data);
        }
    }
    ...
    public function foodTags() {
        ...
        $data = array(); 
        $data['foodTags'] = $this->Master_model->getFoodTags($company_id); 
        $data['main_content'] = $this->load->view('master/foodTag/foodTag', $data, TRUE);
        $this->load->view('userHome', $data);
    }

    public function deleteFoodTag($id) {
        $id = $this->custom->encrypt_decrypt($id, 'decrypt');

        $this->Master_model->deleteFoodTag($id);

        $this->session->set_flashdata('exception', lang('delete_success'));
        redirect('Master/foodTags');
    }

    public function addEditFoodTag($encrypted_id = "") {
        $id = $this->custom->encrypt_decrypt($encrypted_id, 'decrypt');
        
        $data = array();
        $data['types'] = array("Radio Group", "Checkbox Group", "Text Field");
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() == TRUE) { 
                $food_tag_info = array();
                $food_tag_info['title'] = htmlspecialchars($this->input->post($this->security->xss_clean('title')));
                $food_tag_info['type'] = $this->input->post($this->security->xss_clean('type'));
                $food_tag_info['description'] = $this->input->post($this->security->xss_clean('description'));
                $food_tag_info['company_id'] = $this->session->userdata('company_id');
                
                if ($id == "") {
                    $food_tag_id = $this->Common_model->insertInformation($food_tag_info, "tbl_food_tags"); 
                    $this->saveFoodTagItems(
                        $_POST['item_title'],
                    ...
                    $this->Common_model->updateInformation($food_tag_info, $id, "tbl_food_tags");
                    $this->Common_model->deletingMultipleFormData('tag_id', $id, 'tbl_food_tags_items');
                    $this->saveFoodTagItems(
                        $_POST['item_title'],
                        $_POST['item_description'],
                        $_POST['item_price'],
                ...
                if ($id == "") {
                    $data['main_content'] = $this->load->view('master/foodTag/addFoodTag', $data, TRUE);
                    $this->load->view('userHome', $data);
                } else {
                    $data['encrypted_id'] = $encrypted_id;
                    $data['food_tag_details'] = $this->Common_model->getDataById($id, "tbl_food_tags");
                    $data['food_tag_items'] = $this->Master_model->getItemsByTag($id);
                    $data['main_content'] = $this->load->view('master/foodTag/editFoodTag', $data, TRUE);
                    $this->load->view('userHome', $data);
                }
            }
        } else {
    ...
    public function saveFoodTagItems($items_title, $items_description, $items_price, $food_tag_id) {
        for ($r = 0; $r < count($items_title); $r++) {
            $fti = array();
            $fti['tag_id'] = $food_tag_id;
            $fti['title'] = $items_title[$r];
            $fti['description'] = $items_description[$r];
            $fti['price'] = $items_price[$r];
            $fti['company_id'] = $this->session->userdata('company_id');
            $this->Common_model->insertInformation($fti, "tbl_food_tags_items");
        }
    }

    public function foodTagDetails($id) {
        $encrypted_id = $id;
        $id = $this->custom->encrypt_decrypt($id, 'decrypt');
        ...
        $data['food_tag_details'] = $this->Common_model->getDataById($id, "tbl_food_tags");
        $data['food_tag_items'] = $this->Master_model->getItemsByTag($id);
        $data['main_content'] = $this->load->view('master/foodTag/foodTagDetails', $data, TRUE);
        $this->load->view('userHome', $data);
    }
    ...
    public function addEditPaymentMethod($encrypted_id = "") {
        ...
        if($this->input->post('submit')) {
            ...
            $name = htmlspecialchars($this->input->post($this->security->xss_clean('name')));
            if ($name == "IPP") {
                $this->form_validation->set_rules('ipp_endpoint_url', lang('ipp_endpoint_url'), 'required');
                $this->form_validation->set_rules('ipp_username', lang('ipp_username'), 'required');
                $this->form_validation->set_rules('ipp_password', lang('ipp_password'), 'required');
            }
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $fmc_info['ipp_endpoint_url'] = htmlspecialchars($this->input->post($this->security->xss_clean('ipp_endpoint_url')));
                $fmc_info['ipp_username'] = $this->input->post($this->security->xss_clean('ipp_username'));
                $fmc_info['ipp_password'] = $this->input->post($this->security->xss_clean('ipp_password'));
                $fmc_info['ipp_methods'] = $this->array2String($this->input->post($this->security->xss_clean('item_id')));
                ...
                if ($id == "") {
                    ...
                    $data['emv_device_list'] = $this->getEMVDeviceList($data['payment_method_information']);
                    $data['ipp_methods'] = $this->Master_model->getIPPMethods($company_id);
    ...
    private function array2String($item_ids) {
        $ret = "";
        foreach ($item_ids as $row => $item_id):
            $ret .= "||" . $item_id;
        endforeach;

        return strlen($ret) > 0 ? substr($ret, 2) : $ret;
    }
    ...
    private function getEMVDeviceList($ipp_settings) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $ipp_settings->ipp_endpoint_url . "/emv");
        curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
        curl_setopt($ch, CURLOPT_USERPWD, $ipp_settings->ipp_username . ":" . $ipp_settings->ipp_password);
    ...
    public function updateEMVLabel($encrypted_id = "") {
        $id = $this->custom->encrypt_decrypt($encrypted_id, 'decrypt');

        $emv_id = $this->input->post('emv_id');
        $emv_label = $this->input->post('emv_label');
        
        $payment_method_information = $this->Common_model->getDataById($id, "tbl_payment_methods");

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $payment_method_information->ipp_endpoint_url . "/emv");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "id={$emv_id}&label={$emv_label}");
        curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
        curl_setopt($ch, CURLOPT_USERPWD, $payment_method_information->ipp_username . ":" . $payment_method_information->ipp_password);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLINFO_HEADER_OUT, true);
        ...
        redirect("Master/addEditPaymentMethod/{$id}");
    }
